...
int main()
{
    ...
    auto min_val = opt_min(5, 3, 8, 1, 4);
    auto max_val = opt_max(5, 3, 8, 1, 4);
    ...
    auto mixed_min = opt_min(10, maybe_small, missing, 7);
    std::cout << "opt_min(10, optional{2}, nullopt, 7) = " << *mixed_min << "\n";

    // opt_minmax variadic
    auto [lo, hi] = opt_minmax(5, 3, 8, 1, 4);
    std::cout << "opt_minmax(5,3,8,1,4) = {" << *lo << ", " << *hi << "}\n";

    // Range-based variants
    std::vector<int> v{10, 20, 5, 15};
    auto rmin = opt_min_range(v);
    auto rmax = opt_max_range(v);
    auto [rlo, rhi] = opt_minmax_range(v);
    std::cout << "opt_min_range({10,20,5,15}) = " << *rmin << "\n";
    std::cout << "opt_max_range({10,20,5,15}) = " << *rmax << "\n";
    std::cout << "opt_minmax_range({10,20,5,15}) = {" << *rlo << ", " << *rhi << "}\n";
