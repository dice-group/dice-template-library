...
namespace dice::template_library {
	...
	TEST_SUITE("testing of the integral templated tuple reverse") {
		TEST_CASE("ctor") {
			integral_template_tuple_rev<3, -1, Data> tuple{individual_construct, 3, 2, 1, 0};
			REQUIRE(tuple.get<3>().value_ == 3);
			REQUIRE(tuple.get<2>().value_ == 2);
			REQUIRE(tuple.get<1>().value_ == 1);
			REQUIRE(tuple.get<0>().value_ == 0);

			integral_template_tuple_rev<3, -1, Data> tuple2{uniform_construct, 0};
			REQUIRE(tuple2.get<3>().value_ == 0);
			REQUIRE(tuple2.get<2>().value_ == 0);
			REQUIRE(tuple2.get<1>().value_ == 0);
			REQUIRE(tuple2.get<0>().value_ == 0);

			integral_template_tuple_rev<3, -1, Data> copy{tuple};
			tuple = copy;
			tuple = std::move(copy);

			integral_template_tuple_rev<3, -1, Data> moved{std::move(tuple2)};
		}

		TEST_CASE("comparision") {
			integral_template_tuple_rev<3, -1, Data> tuple{individual_construct, 3, 2, 1, 0};
			integral_template_tuple_rev<3, -1, Data> tuple2{individual_construct, 4, 3, 2, 1};

			REQUIRE(tuple <=> tuple2 == std::strong_ordering::less);
		}

		TEST_CASE("Is same for single value at 4") {
			integral_template_tuple_rev<4, 3, Data> inTuple;
			auto vec = to_int_vector<4, 5>(inTuple);
			REQUIRE(is_equal(vec, {4}));
		}

		TEST_CASE("Is same for multiple values counting down") {
			integral_template_tuple_rev<8, 3, Data> inTuple;
			auto vec = to_int_vector<4, 9>(inTuple);
			REQUIRE(is_equal(vec, {4, 5, 6, 7, 8}));
		}

		TEST_CASE("Negative values counting down") {
			integral_template_tuple_rev<2, -3, Data> inTuple;
			auto vec = to_int_vector<-2, 3>(inTuple);
			REQUIRE(is_equal(vec, {-2, -1, 0, 1, 2}));
		}

		TEST_CASE("reinterpret_cast counting down") {
			integral_template_tuple_rev<3, -1, Data> inTuple;
			// NOLINTNEXTLINE
			integral_template_tuple_rev<2, 0, Data> &casted = inTuple.template subtuple<2, 0>();
			auto vec = to_int_vector<1, 3>(casted);
			REQUIRE(is_equal(vec, {1, 2}));
		}

		TEST_CASE("reinterpret_cast counting down into negative") {
			integral_template_tuple_rev<3, -3, Data> inTuple;
			// NOLINTNEXTLINE
			integral_template_tuple_rev<2, -2, Data> &casted = inTuple.template subtuple<2, -2>();
			auto vec = to_int_vector<-1, 3>(casted);
			REQUIRE(is_equal(vec, {-1, 0, 1, 2}));
		}

		TEST_CASE("size") {
			REQUIRE(integral_template_tuple_rev<1, -2, Data>::size() == 3);
			REQUIRE(integral_template_tuple_rev<2, 1, Data>::size() == 1);
			REQUIRE(integral_template_tuple_rev<3, 1, Data>::size() == 2);
		}

		TEST_CASE("visit") {
			integral_template_tuple_rev<2, -1, Data> const tuple{};
